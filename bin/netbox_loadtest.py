...
def carve_new_prefix(parent_prefix: str, prefix_length: int) -> dict:
    ...
    result = session.post(
        f"{ URL }/ipam/prefixes/{ id }/available-prefixes/", data=json.dumps(data)
    )
...
def get_and_reserve_next_free_address(prefix: dict) -> dict:
    ...
    id = prefix["id"]
    result = session.post(
        f"{ URL }/ipam/prefixes/{ id }/available-ips/", data=json.dumps(data)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "workers", type=int, help="number of workers concurrenting working"
    )
